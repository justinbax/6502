...
struct CPU {
	...
	void reset(uint32_t &cycles, MEMORY &mem) {
		...
		rwStack(mem, READ);
		rwStack(mem, READ);
		// stores $0xFFFD in the low byte of the program counter register and decrements stack pointer
		reg_programCounter = rwPage(mem, reg_stackPointer, 0xFF, READ, 0x00);
		reg_stackPointer--;
		// stores $0xFFFC in the high byte of the program counter register and decrements stack pointer
